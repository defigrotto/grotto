...
contract Grotto {
    ...
    address public constant MAINNET_ETH_USD_PF =
        0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419;
    address public constant KOVAN_ETH_USD_PF =
        0x9326BFA02ADD2366b30bacB125260Af641031331;
    address public constant BSC_ETH_USD_PF =
    ...
    constructor(address _tokenAddress, address _storeAddress) {
        ...
        grottoToken.setGrotto(address(this), msg.sender);

        store = StorageInterface(storeAddress);
        store.setGrotto(address(this), msg.sender);
        ...
        grottoToken.mintToken(
            0xE447B45EA6FD4c8B76b25354a503C793826e5294,
            (store.getMinGrottoGovernor() * 10)
        );
    }

    function getRewardPerGrotto(uint256 stakePoolIndex)
        public
        view
        returns (uint256)
    {
    ...
    function stake(uint256 amount) public {
        ...
        require(
            grottoToken.balanceOf(staker) >= amount,
            "You do not have enough GROTTO"
        );
    ...
    function getStakeInPool(address staker, uint256 stakePoolIndex)
        public
        view
        returns (uint256)
    {
    ...
    function getGrottoTokenBalance(address account)
        public
        view
        returns (uint256)
    {
    ...
    function processShares() public {
        if (store.getPendingGrottoMintingPayments() >= 10 ether) {
            ...
            uint256 totalStakedForCurrentPool =
                store.getStake(
                    store.getStakingMaster(),
                    store.getCurrentStakePoolIndex()
                );
            // multiply by 1 ether so that result will not be 0 for result like (0.12/60000)
            uint256 ethPerGrotto =
                toStakers.mul(1 ether).div(totalStakedForCurrentPool);
            store.setRewardPerGrotto(
                store.getCurrentStakePoolIndex(),
                ethPerGrotto
            );
            store.setPendingGrottoMintingPayments(0);
            store.addCompletedPool(store.getCurrentStakePoolIndex());
            store.setCurrentStakePoolIndex(
                store.getCurrentStakePoolIndex().add(1)
            );
    ...
    function enterPool(bytes32 poolId) public payable {
        ...
        require(
            store.getPoolers(poolId).length < pool.poolSize,
            "This Pool is full"
        );

        require(
            pooler != pool.poolCreator,
        ...
        require(
            usdtValue >= pool.poolPrice,
            "Value Sent is less than pool price."
        );
    ...
    function startNewPool(uint256 poolSize, bytes32 poolId) public payable {
        ...
        if (pooler == store.getHouse()) {
            ...
            require(
                poolSize >= store.getMinimumPoolSize(),
                "Pool size too low"
            );
            require(
                poolSize <= store.getMaximumPoolSize(),
                "Pool size too high"
            );
        }
    ...
    function _payWinner(bytes32 poolId) internal {
        ...
        uint256 mid = poolers.length / 2;
        uint256 end = poolers.length - 1;
        ...
        uint256 winnerIndex =
            uint256(keccak256(abi.encodePacked(totalStaked, randBase))) %
                (pool.poolSize);
        ...
        uint256 houseCutNewTokens =
            creatorReward.mul(store.getHouseCutNewTokens()).div(100);
        grottoToken.mintToken(
            pool.poolCreator,
            creatorReward.sub(houseCutNewTokens)
        );
        grottoToken.mintToken(store.getHouse(), houseCutNewTokens);

        store.setPendingGrottoMintingPayments(
            store.getPendingGrottoMintingPayments().add(pool.poolPriceInEther)
        );
